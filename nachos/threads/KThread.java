...
public class KThread {
	...
	public static void finish() {
		...
		Machine.interrupt().disable();//���ж�
		Machine.autoGrader().finishingCurrentThread();//��TCB��ɽ�Ҫ������TCB
		
		Lib.assertTrue(toBeDestroyed == null);
		
		toBeDestroyed = currentThread;//����ǰ�̱߳�ɽ�Ҫ�������̣߳���һ���߳����е�ʱ���Զ�������
...
//		new PingTest(0).run();
	...
	public static void joinTest(){
		...
		//考虑下面的问题，PingTest类中有一句currentThread.yield();是什么意思˼
		...
		KThread threadB = new KThread(new Runnable() {
			public void run() {
				...
				threadA.join();
				//考虑下面这个问题，为什么要让currentThread yield
				currentThread.yield();
	...
	public static void conditionTest(){
		...
		KThread kt1 = new KThread(new Runnable() {
			...
			public void run() {
				...
				condition2.sleep();
				System.out.println("kt1 is running");
				condition2.wake();
				lock.release();
				System.out.println("kt1 is OVER");
			}
		});
		kt1.fork();
		KThread kt2 = new KThread(new Runnable() {
			...
			public void run() {
				...
				KThread.currentThread().yield();
				condition2.wake();
				System.out.println("kt2 is running");
				condition2.sleep();
				lock.release();
				System.out.println("kt2 is OVER");
			}
		});
	...
	public static void alarmTest(){
		...
		new KThread(new Runnable() {
			@Override
			public void run() {
				System.out.println(" 睡眠前系统时间："+Machine.timer().getTime()+"\n1线程开始睡眠，时间500");
			    new Alarm().waitUntil(1000);
				System.out.println("1醒来后系统时间："+Machine.timer().getTime());
			}
		...
		new KThread(new Runnable() {
			...
			public void run() {
				System.out.println(" 睡眠前系统时间："+Machine.timer().getTime()+"\n2线程开始睡眠，时间500");
			    new Alarm().waitUntil(1000);
				System.out.println("2醒来后系统时间："+Machine.timer().getTime());
			}
	...
	//考虑下面的这个waitQueue 是否是要使用static
	private static ThreadQueue waitQueue = ThreadedKernel.scheduler.newThreadQueue(true);// �ȴ�����
